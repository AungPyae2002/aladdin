                  ...
                  <table class="table">
                    ...
                    <tbody>
                      @foreach ($schedules as $index=>$schedule)
                          <tr>
                              <td class="">{{$index+1}}</td>
                              <td>{{$schedule->type->name ?? null}}</td>
                              <td class="pb-0">{{Carbon\Carbon::parse($schedule->opening_date_time)->format('h:i A')}} ({{$schedule->multiply}}z)</td>
                              <td class="pb-0">
                              ...
                              <td class="pb-0">{{$schedule->set}}</td>
                              <td class="pb-0">{{$schedule->value}}</td>

                              <td class="pb-0">
                                  <a href="{{route('admin.twod_section.edit',$schedule->id)}}" class="btn btn-sm btn-info">
                                  ...
                                  <form id="type-delete-{{$schedule->id}}" action="{{route('admin.twod_section.destroy',$schedule->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                  </form>
                              </td>
